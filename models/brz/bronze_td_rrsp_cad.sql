{{ config(
    ...
    unique_key='TRAN_TD_CAD_RRSP_INVEST_TRANSACTIONS_hKey',
...
with src as (
    select
        {{ dbt_utils.generate_surrogate_key([
            "upper(trim(SYMBOL))",
            "upper(trim(MARKET))"
        ]) }} as TRAN_TD_CAD_RRSP_INVEST_TRANSACTIONS_hKey,
...
latest_target as (
    {% if is_incremental() %}
    select
        TRAN_TD_CAD_RRSP_INVEST_TRANSACTIONS_hKey,
        max(STAGING_INSERT_TIME) as max_time
    from {{ this }}
    group by 1
    {% else %}
    select null as TRAN_TD_CAD_RRSP_INVEST_TRANSACTIONS_hKey, null as max_time
    {% endif %}
),
...
latest_hash as (
    {% if is_incremental() %}
    select
        t.TRAN_TD_CAD_RRSP_INVEST_TRANSACTIONS_hKey,
        t.TRAN_TD_CAD_RRSP_INVEST_TRANSACTIONS_hdiff
    from {{ this }} t
    qualify row_number() over (
        partition by TRAN_TD_CAD_RRSP_INVEST_TRANSACTIONS_hKey
        order by STAGING_INSERT_TIME desc
    ) = 1
    {% else %}
    select
        null as TRAN_TD_CAD_RRSP_INVEST_TRANSACTIONS_hKey,
...
left join latest_hash h
  on s.TRAN_TD_CAD_RRSP_INVEST_TRANSACTIONS_hKey = h.TRAN_TD_CAD_RRSP_INVEST_TRANSACTIONS_hKey
where
    -- first time seen OR changed attributes
    h.TRAN_TD_CAD_RRSP_INVEST_TRANSACTIONS_hKey is null
